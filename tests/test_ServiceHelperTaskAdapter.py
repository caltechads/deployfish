...
class BaseTestServiceHelperTaskAdapter_basic:
    ...
    def test_tasks_have_correct_family(self):
        data_list, kwargs_list = self.adapter.convert()
        assert kwargs_list[0]["task_definition"].data["family"] == "foobar-tasks-test-migrate"
        assert kwargs_list[1]["task_definition"].data["family"] == "foobar-tasks-test-update-index"
    ...
    def test_tasks_have_appropriate_family_set_if_not_supplied(self):
        ...
        assert kwargs_list[0]["task_definition"].data["family"] == "foobar-test-tasks-migrate"
        assert kwargs_list[1]["task_definition"].data["family"] == "foobar-test-tasks-update-index"
    ...
    def test_containers_should_have_no_ports(self):
        data_list, kwargs_list = self.adapter.convert()
        assert "portMappings" not in kwargs_list[0]["task_definition"].containers[0].data
        assert "portMappings" not in kwargs_list[1]["task_definition"].containers[0].data

    def test_task_containers_have_correct_command(self):
        data_list, kwargs_list = self.adapter.convert()
        assert kwargs_list[0]["task_definition"].containers[0].data["command"] == ["manage.py", "migrate"]
        assert kwargs_list[1]["task_definition"].containers[0].data["command"] == ["manage.py", "update_index"]
    ...
    def test_DEPLOYFISH_TASK_NAME_in_container_environment(self):
        ...
        env0 = kwargs_list[0]["task_definition"].containers[0].data["environment"]
        env1 = kwargs_list[1]["task_definition"].containers[0].data["environment"]
        assert {"name": "DEPLOYFISH_TASK_NAME", "value": "foobar-tasks-test-migrate"} in env0
        assert {"name": "DEPLOYFISH_TASK_NAME", "value": "foobar-tasks-test-update-index"} in env1

    def test_DEPLOYFISH_ENVIRONMENT_set_correctly_in_container_environment(self):
        data_list, kwargs_list = self.adapter.convert()
        env0 = kwargs_list[0]["task_definition"].containers[0].data["environment"]
        env1 = kwargs_list[1]["task_definition"].containers[0].data["environment"]
        deployfish_env = {"name": "DEPLOYFISH_ENVIRONMENT", "value": self.service.deployfish_environment}
    ...
    def test_DEPLOYFISH_CLUSTER_NAME_set_correctly_in_container_environment(self):
        ...
        env0 = kwargs_list[0]["task_definition"].containers[0].data["environment"]
        env1 = kwargs_list[1]["task_definition"].containers[0].data["environment"]
        assert {"name": "DEPLOYFISH_CLUSTER_NAME", "value": data_list[0]["cluster"]} in env0
        assert {"name": "DEPLOYFISH_CLUSTER_NAME", "value": data_list[1]["cluster"]} in env1
...
class TestServiceHelperTaskAdapter_FARGATE(BaseTestServiceHelperTaskAdapter_basic):
    ...
    def test_tasks_have_vpc_configuration(self):
        ...
        for data in data_list:
            ...
            assert data["networkConfiguration"]["awsvpcConfiguration"] == { "subnets": ["subnet-1", "subnet-2"], "securityGroups": ["sg-1", "sg-2"], "assignPublicIp": "ENABLED" }
    ...
    def test_commands_can_have_their_own_vpc_configuration(self):
        ...
        assert data_list[0]["networkConfiguration"]["awsvpcConfiguration"] == { "subnets": ["subnet-1", "subnet-2"], "securityGroups": ["sg-1", "sg-2"], "assignPublicIp": "ENABLED" }
        assert kwargs_list[0]["task_definition"].data["requiresCompatibilities"] == ["FARGATE"]
        ...
        assert kwargs_list[1]["task_definition"].data["requiresCompatibilities"] == ["EC2"]
...
class TestServiceHelperTaskAdapter_schedule_EC2:
    ...
    def test_schedule_rule_name_is_correct(self):
        assert self.kwargs_list[0]["schedule"].name == "deployfish-foobar-tasks-test-migrate"

    def test_schedule_rule_ScheduleExpression_is_correct(self):
        assert self.kwargs_list[0]["schedule"].data["ScheduleExpression"] == "cron(5 * * * ? *)"

    def test_schedule_rule_cluster_arn_is_correct(self):
        assert self.kwargs_list[0]["schedule"].target.data["Arn"] == "MY_REAL_CLUSTER_ARN"

    def test_schedule_rule_RoleArn_is_correct(self):
        assert self.kwargs_list[0]["schedule"].target.data["RoleArn"] == "MY_SCHEDULE_ROLE"

    def test_schedule_rule_TaskCount_is_correct(self):
        assert self.kwargs_list[0]["schedule"].target.data["EcsParameters"]["TaskCount"] == 1

    def test_schedule_rule_LaunchType_is_correct(self):
        assert self.kwargs_list[0]["schedule"].target.data["EcsParameters"]["LaunchType"] == "EC2"

    def test_schedule_rule_Group_is_not_set(self):
        assert "Group" not in self.kwargs_list[0]["schedule"].target.data["EcsParameters"]

    def test_schedule_rule_NetworkConfiguration_is_not_set(self):
        assert "NetworkConfiguration" not in self.kwargs_list[0]["schedule"].target.data["EcsParameters"]
...
class TestServiceHelperTaskAdapter_schedule_FARGATE:
    ...
    def test_schedule_rule_LaunchType_is_correct(self):
        assert self.kwargs_list[0]["schedule"].target.data["EcsParameters"]["LaunchType"] == "FARGATE"

    def test_schedule_rule_Group_is_not_set(self):
        assert "Group" not in self.kwargs_list[0]["schedule"].target.data["EcsParameters"]

    def test_schedule_rule_NetworkConfiguration_is_set(self):
        assert "NetworkConfiguration" in self.kwargs_list[0]["schedule"].target.data["EcsParameters"]
