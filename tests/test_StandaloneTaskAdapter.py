...
class BaseTestStandaloneTaskAdapter_basic:
    ...
    def test_secrets_are_set_properly_in_task_definition(self):
        ...
        assert len(td_secrets) == len(self.CONFIG["config"])
...
class TestStandaloneTaskAdapter_schedule_EC2:
    ...
    def test_schedule_rule_is_returned(self):
        assert "schedule" in self.kwargs

    def test_schedule_rule_name_is_correct(self):
        assert self.kwargs["schedule"].name == "deployfish-foobar-test-mytask"

    def test_schedule_rule_ScheduleExpression_is_correct(self):
        assert self.kwargs["schedule"].data["ScheduleExpression"] == "cron(5 * * * ? *)"

    def test_schedule_rule_cluster_arn_is_correct(self):
        assert self.kwargs["schedule"].target.data["Arn"] == "MY_REAL_CLUSTER_ARN"

    def test_schedule_rule_RoleArn_is_correct(self):
        assert self.kwargs["schedule"].target.data["RoleArn"] == "MY_SCHEDULE_ROLE"
