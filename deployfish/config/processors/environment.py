...
class EnvironmentConfigProcessor(AbstractConfigProcessor):
        ...
        try:
            ...
            if e.errno == errno.EACCES:
                if not self.context.get("ignore_missing_environment", False):
                    ...
                    raise self.ProcessingFailed(msg) from e
                return {}
    ...
    def replace(
        ...
        # TODO: need to deal with multiple matches in the same line
        ...
        if m:
            ...
            try:
                ...
                try:
                    ...
                    if not self.context.get("ignore_missing_environment", False):
                        msg = f'Config["{section_name}"]["{item_name}"]: Could not find value for ${{env.{envkey}}}'  # noqa: E501
