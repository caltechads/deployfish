...
class TestApplicationAutoscaling_load_yaml_target_tracking(unittest.TestCase):

    def setUp(self):
        self.init = Mock(return_value=None)
        with Replacer() as r:
            r.replace('deployfish.aws.appscaling.ScalingPolicy.__init__', self.init)
            self.appscaling = ApplicationAutoscaling('my_service', 'my_cluster', YML_TargetTracking)
    ...
    def test_policies(self):
        self.assertEqual(len(self.appscaling.policies), 1)
        self.assertTrue('scale-target' in self.appscaling.policies)
        self.init.assert_any_call('my_service', 'my_cluster', YML['scale-target'])
...
class TestApplicationAutoscaling__render_create_target_tracking(unittest.TestCase):
